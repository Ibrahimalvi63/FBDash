    ...
    return (
        <div className="md:w-30 text-gray-300 bg-violet-950 md:bg-blue-900/10 md:border md:border-blue-500/20 md:px-2 md:py-40 fixed left-0 bottom-0 w-full md:h-screen md:flex md:flex-col z-50">
            ...
            <ul className={`md:block flex items-center justify-around md:gap-10`}>
                ...
                <li onClick={() => setActive('orders')} className={`flex-1 md:mb-3 ${active == '/dashboard/orders' ? 'bg-blue-500/50' : ''} ${active == 'orders' ? 'bg-blue-500/50' : ''} rounded text-center p-1 text-xl`}><Link href={'/dashboard/orders'}><div className="flex items-center gap-1 flex-col md:flex-row"><div>📦</div><div onClick={optionHide} className="text-xs">Orders</div></div></Link></li>
                <li onClick={() => setActive('customers')} className={`flex-1 md:mb-3 ${active == '/dashboard/customers' ? 'bg-blue-500/50' : ''} ${active == 'customers' ? 'bg-blue-500/50' : ''} rounded text-center p-1 text-xl`}><Link href={'/dashboard/customers'}><div className="flex items-center gap-1 flex-col md:flex-row"><div>👤</div><div onClick={optionHide} className="text-xs">Customers</div></div></Link></li>
